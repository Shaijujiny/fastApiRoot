...
from app.api.auth.schema import AdminRegisterRequest, ProfileResponse, TokenData, UserRegisterRequest

from app.depends.jwt_depends import JWTService
from app.models.postgresql.users import TblUser, UsersBaseModel as PgUserBase
from app.models.mysql.admin import TblAdmin, AdminBaseModel as MyUserBase
...
class AuthService:
    ...
    async def user_login(data, db: AsyncSession, lang: str):
        ...
        access_token = await JWTService().create_access_token(
            user_id=user.id, role=user.role
        )
        refresh_token = await JWTService().create_refresh_token(
            user_id=user.id, role=user.role
        )
